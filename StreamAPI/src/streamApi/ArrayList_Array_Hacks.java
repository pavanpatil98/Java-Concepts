...
class ArrayList_Array_Hacks {
	...
	public void ArrayList1DCustomToArray() {
		...
		List<Pair> ans = arrPair.stream().sorted((a,b)->{Integer r = a.start;Integer s=b.start; return r.compareTo(s);}).collect(Collectors.toList());
		 //List<Interval> sortedList = intervals.stream().sorted((a,b)->{Integer p = a.start;Integer q = b.start;return p.compareTo(q);}).collect(Collectors.toList());
